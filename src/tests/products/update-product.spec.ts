import { beforeEach, describe, expect, it } from 'vitest'
import { ProductsRepository } from '../../repositories/products-repository'
import { InMemoryProductsRepository } from '../../repositories/in-memory-repository/in-memory-products-repository'
import { Decimal } from '@prisma/client/runtime/library'
import { ProductNotFounderError } from '../../errors/product-not-founder-error'
import { UpdateProductsUseCase } from '../../use-cases/products/update-products-use-case'

let productsRepository: ProductsRepository
let sut: UpdateProductsUseCase

describe('Update Products Use Case', () => {
  beforeEach(() => {
    productsRepository = new InMemoryProductsRepository()
    sut = new UpdateProductsUseCase(productsRepository)
  })

  it('should be able udpdate a product', async () => {
    const procuctCreated = await productsRepository.create({
      name: 'Product 1',
      description: 'Teste description',
      price: new Decimal(1.5),
      stock: 5,
    })

    const productUpdated = await sut.execute({
      id: procuctCreated.id,
      name: 'Product 2',
      description: 'Teste description',
    })

    await expect(productUpdated).toEqual(expect.objectContaining({}))
  })

  it('should be not able update a product inexistent', async () => {
    await productsRepository.create({
      name: 'Product 1',
      description: 'Teste description',
      price: new Decimal(1.5),
      stock: 5,
    })

    await expect(async () => {
      await sut.execute({ id: '123' })
    }).rejects.toBeInstanceOf(ProductNotFounderError)
  })
})
